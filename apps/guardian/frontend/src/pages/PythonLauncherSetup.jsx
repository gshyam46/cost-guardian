...
export default function PythonLauncherSetup() {
  ...
  const code = (name, label) => <div className="rounded-lg border bg-ink-50 overflow-hidden"><div className="flex items-center justify-between gap-3 border-b px-4 py-2"><span className="text-xs font-medium text-ink-600">{label}</span><Button size="sm" variant="ghost" onClick={() => copy(name)} aria-label={`Copy ${name} commands`}>Copy</Button></div><pre aria-label={label} tabIndex={0} className="p-4 text-xs overflow-x-auto whitespace-pre-wrap break-words"><code>{commands[name]}</code></pre></div>;
  return <section aria-labelledby="python-launcher-title" className="space-y-5 text-sm text-ink-600">
    <div><h2 id="python-launcher-title" className="text-lg font-semibold text-ink-900">Install once. Run your app with Sillage.</h2><p className="mt-2">Capture supported Python model calls without copying helper files or editing every call. Your application keeps its existing provider client, credentials and responses.</p><p className="mt-2 text-xs">OpenInference captures supported SDK or framework calls; Sillage preserves their trace and parent IDs. Agent names require supplied context. An unlabelled app uses your service name.</p></div>
    <div className="flex flex-wrap gap-4"><div><label htmlFor="launcher-shell" className="text-xs font-medium text-ink-700">Terminal</label><select id="launcher-shell" className="mt-1 block rounded-md border bg-card px-3 py-2 text-sm" value={shell} onChange={event => change(setShell, event.target.value)}><option value="powershell">PowerShell</option><option value="bash">Bash / zsh</option></select></div><div><label htmlFor="launcher-target" className="text-xs font-medium text-ink-700">How you start your app</label><select id="launcher-target" className="mt-1 block rounded-md border bg-card px-3 py-2 text-sm" value={target} onChange={event => change(setTarget, event.target.value)}><option value="uvicorn">Uvicorn / FastAPI</option><option value="script">Python script</option></select></div></div>
    <div className="flex flex-wrap gap-4"><div><label htmlFor="launcher-capture" className="text-xs font-medium text-ink-700">Capture method</label><select id="launcher-capture" className="mt-1 block rounded-md border bg-card px-3 py-2 text-sm" value={capture} onChange={event => change(setCapture, event.target.value)}><option value="openinference">OpenInference / OpenTelemetry</option><option value="native">Native compatibility</option></select></div>{capture === 'openinference' && <div><label htmlFor="launcher-layer" className="text-xs font-medium text-ink-700">Library your app calls</label><select id="launcher-layer" className="mt-1 block rounded-md border bg-card px-3 py-2 text-sm" value={layer} onChange={event => change(setLayer, event.target.value)}><option value="auto">Detect one compatible layer</option><option value="openai">OpenAI</option><option value="litellm">LiteLLM</option><option value="langchain">LangChain</option></select></div>}</div>
