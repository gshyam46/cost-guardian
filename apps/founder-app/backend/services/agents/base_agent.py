...
Observability: each agent's LLM call is traced in Langfuse (see llm_fallback.py);
this module additionally logs timing/status locally on AgentContext.agent_traces.
"""
import json
import logging
...
class AgentContext(BaseModel):
    ...
    # Groups every agent's LLM calls in one pipeline execution into a single
    # Langfuse trace (see llm_fallback.LlmChat.send_message). Set once by the
    # orchestrator at the start of a run, not per-agent.
...
class BaseAgent(ABC):
    ...
    async def run(self, context: AgentContext) -> AgentContext:
        ...
        try:
            ...
            # LlmChat.send_message() already rotates models on transport errors, but a
            # model that answers successfully with malformed JSON slips past that and
            # would kill the whole 5-agent pipeline. Observed in practice: the same
            # prompt parsed fine on retry, so this is intermittent model output, not a
            # bad prompt -- re-asking is the right response.
            response = ""
            for attempt in range(1, JSON_PARSE_ATTEMPTS + 1):
                ...
                try:
                    ...
                    if attempt == JSON_PARSE_ATTEMPTS:
                        raise
                    logger.warning(
                        f"[{self.name}] Unparseable JSON on attempt {attempt}"
                        f"/{JSON_PARSE_ATTEMPTS} ({parse_error}); re-asking."
                    )
