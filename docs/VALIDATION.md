...
| Check | Current result | Scope / report |
| --- | --- | --- |
| Complete frontend suite | **396 passed**, 13 suites, zero failed/skipped; 74.660 s | `tools/reports/design-frontend.json`; existing locked dependencies in an isolated source stage. |
| Public function and build contracts | **29 passed**, 411.890 ms | `node --test apps/guardian/frontend/server/public.test.cjs apps/guardian/frontend/scripts/test-public-build.cjs`; 23 storage/server and six build/config tests, including the explicit waitlist rewrite. |
| Backend static boundary | **53 passed, one platform skip**, 11.05 s | `pytest tests/test_deployment_static.py -q -p no:cacheprovider` from the backend with `PYTHON_DOTENV_DISABLED=1`; fixed brand/license paths outside the CRA manifest, immutable snapshots, bounded files and private-path rejection. The skip requires Windows symlink privileges. |
| Workspace browser scenarios | **69 passed** | `tools/reports/browser-design/report.json`; built workspace against synthetic API/identity transports. |
| Actual public registration | **Ten phases passed**, 49.276 s | `tools/reports/public-launch-0af7e975fa/report.json`; real Node HTTP handlers, Mongo 8.0.26 transactions and Edge. |
| Native deployment/onboarding | **Eight phases passed**, 58.124 s | `tools/reports/experience-native/report.json`; actual local OIDC, API/worker, Mongo, browser, restart and transport recovery. |
| Deployment harness unit checks | **Nine passed**, 6.88 s | Focused `tools/tests/test_deployment_harness.py` suite after the cleanup diagnostic repair. |
| Running preview layout | **12 public page/viewport checks passed** | `tools/reports/design-preview/report.json`; `/`, `/signup`, `/waitlist`, `/privacy` at 320, 390 and 1280 pixels, plus actual workspace welcome/sign-in and brand asset checks. |

Both public and workspace builds use **75 explicit input hashes** and produce **14 recorded artifacts**, including three source-matching WOFF2 files totalling 67,996 bytes. The final public build passed in **52.769 seconds**; workspace in **29.538 seconds**. Reports `tools/reports/design-public-build.json` and `design-workspace-build.json` confirm stable source hashes, matching icons/licenses, required CSS/font faces, no source maps, no synthetic dotenv/server/ambient canaries and no external font URLs. Building workspace mode leaves the public artifact unchanged. The public privacy contact is deliberately `privacy@example.test` for local acceptance, not a production contact. The first public build was superseded after the final input-border contrast change.

| Artifact | Public mode | Workspace mode |
| --- | --- | --- |
| Build directory under `.cache/public-dependencies/frontend-clean-verified/` | `build` | `workspace-build` |
| JavaScript | `main.2787a71a.js` | `main.c724452c.js` |
| JavaScript SHA256 | `69266d80a6f25179548358b3965f26910d18fcf75eda0d91acdf99defe7d7f69` | `bc8bba3dddbe2de42a2786f9f0348e615269f33e258c54090a01d8d003276ccb` |
| Manifest SHA256 | `3afefc80ddb0e8fae9ad41d0fb494f15bed47cc77040f58a49830d98e16a44ed` | `3b787cca9d4502211cfb63580cadc90fb70540a899624e35cfc8ce8486b10f2f` |

Both modes share `main.d9773048.css`, SHA256 `72b1f4c0d6b5def4315127095f7960f208e23642a825541b88420d2ce4dd1249`. The font sources, licenses and hashes are recorded in [BRAND.md](BRAND.md). Docker COPY/ignore rules and CI clean staging now include the exact icons, font licenses and WOFF2 sources. Backend serving snapshots only the fixed public asset allowlist through the existing size/symlink guards. The current native API served the final fonts/icons successfully. This is packaging and native-runtime evidence; no Docker image was built or run in this slice.

The public acceptance journey exercised the actual committed registration and waitlist navigation, direct/reloaded waitlist neutrality, optional company/project inputs, retained inputs after failed writes, duplicate concurrency, durable rate rollback, a new database client, private export/deletion, expiry renewal and missing/down/hanging/recovered workspace availability. The landing and demo remain independent of protected APIs. Test contacts and databases were synthetic and ownership-marked; owned browser/server/Mongo fixtures were cleaned.

Native acceptance completed two real local authorization/token/callback flows, owner key creation, separate test receipt, a real sender/API/worker event with **10 known tokens and unknown cost**, incident resolution, immutable replay and logout. API/worker restart preserved accounting and resolution; cutting Mongo transport returned readiness 503, and restoration recovered readiness. The existing 0.2.0 wheel download remained hash-identical. There were zero paid model calls or external customer services.

An initial native run completed the first journey but failed during restart-browser cleanup. The old handler discarded the underlying close exception and labelled every cleanup failure a timeout, so the original close cause cannot be recovered. The harness now preserves the journey outcome, records separate bounded context/connection/server closes and requires matching closure events and confirmed owned-process exit. Unknown failures and timeouts still fail acceptance. The complete rerun passed: first cleanup took 21/1/770 ms and restart cleanup 16/1/323 ms, all closure events confirmed, no fallback, and both owned Mongo fixtures removed. The initial report is retained as `tools/reports/experience-native/report-before-cleanup-diagnostic.json`.

The preview check found no horizontal overflow, no external requests, no mutations and no browser errors. Font loading passed at every viewport. Desktop landing, mobile waitlist, workspace Live/Overview, 320px landing/signup and workspace sign-in screenshots were physically inspected. Input borders use `#948071` (about 3.32:1 on cream) and retain visible keyboard focus; muted body text is about 4.86:1 and brick primary text contrast about 5.78:1. These focused checks are not a comprehensive accessibility certification.

Final integrity checks passed with no findings across **161 Python files**, **39 active Markdown files**, **79 code fences**, **523 local links**, source whitespace and both workflows using actionlint **1.7.11**. Report: `tools/reports/deployment-integrity.json`, checked 2026-09-14 19:04:46 UTC. Staged whitespace checking excludes the two byte-preserved upstream OFL license files, each containing an original trailing space on line 21; application source and documentation pass. External documentation links were not fetched. An independent final review found no material registration, routing or asset-packaging blocker.

The workspace preview was refreshed at **2026-09-14 18:55:44 UTC**, retaining `guardian_preview_mvp2`, the existing Mongo/identity dependencies, sessions and workers. Only the identified old API/launcher were replaced; the new API PID is **31532**, launcher **20608**. Record: `.cache/preview/experience-20260914T185456Z-7ce4626d/refresh.json`. The final build is reachable at `http://127.0.0.1:8001/welcome` and actual OIDC sign-in remains available.

A separate public preview runs at `http://127.0.0.1:3004` (Node PID **32072**), backed by the ownership-marked local `sillage_public_preview` database on the existing Mongo. It uses the actual registration store and a loopback-only preview adapter; production origin/TLS validation is unchanged. No registration was submitted by the read-only preview check. The occupied port 3003 and unrelated Founder/AIDA services were preserved. Public registrations entered here remain local and the privacy contact is synthetic. Vercel account/project, a hosted dedicated interest database and a real monitored privacy contact remain unconfigured; no hosted launch, authenticated account creation or managed workspace provisioning is claimed.

## Public Vercel frontend and independent registration: 2026-09-14

ADR-52 and [PUBLIC_LAUNCH.md](PUBLIC_LAUNCH.md) recorded the architecture before code. The public build now includes landing/demo, consented early-access registration, privacy notice and bounded workspace-availability handling. Ordinary builds retain the existing guarded workspace routes. The availability and registration functions operate separately from the monitoring API/worker.

Clean isolated `npm ci` from the updated frontend lockfile passed; existing source `node_modules` was not changed. Node **24.20.0**, MongoDB driver **7.6.0** and `@vercel/functions` **3.9.7** were used. The full frontend suite passed **380 tests across 13 suites**, zero failures/skips, in **11.747 seconds**. The real public build passed in **12.767 seconds**, with **63 source-input hashes**, five artifact hashes, no source maps, required CSS and no synthetic dotenv/server/ambient canaries in its output. Reports: `tools/reports/public-clean-frontend.json` and `public-production-build.json`.

Final public assets are **`main.13dcb676.js`** and **`main.caa19a36.css`** in `.cache/public-dependencies/frontend-clean-verified/build`. Manifest SHA256: **`7c1b68ceb9be83856ce9ba7c0fddd4bffe4579533530e3373f3378ba48482e48`**. JavaScript SHA256: `cb9ddf37569b01d28abd71c2afaf6f7f2ba85ee71d94d542ecc23094bd403c5c`; CSS SHA256: `2488d7663f7a72ac33ef869548ac9eec424a58a54276b8516e572b64f02cc65e`. The build's `privacy@example.test` contact is explicitly synthetic acceptance configuration, not a production contact. The source build removes all server variables and ambient `REACT_APP_*` values except a validated public contact.

`node --test apps/guardian/frontend/server/public.test.cjs apps/guardian/frontend/scripts/test-public-build.cjs` passed **29 tests** (23 server/storage/admin and six build/config) in **387.648 ms**. These cover raw-wire byte/UTF-8/deadline validation, exact origins/schema/consent, fixed non-PII errors, acknowledgment after transaction commit, compatible bootstrap replay and incompatible-schema refusal, original contact preservation, TTL/expiry, counter rollback, HTTPS readiness parsing/timeouts, formula-safe private export, exact deletion and inert CLI help. Build tests execute a real child process and prove secret isolation, untouched dependency caches, linked-output refusal and explicit Vercel route/upload configuration.

The final built site passed **nine actual browser/HTTP/Mongo phases in 7.660 seconds**, using Edge and a new owned Mongo **8.0.26** replica on loopback port 27021. [The reusable harness](../tools/test_public_launch.cjs) exercises the actual Node handlers and transaction store, not substituted registration responses. It verified independent public landing/demo; missing/down/hanging/recovered workspace states; explicit safe sign-in/setup links; committed registration with consent during backend outage; failed-store retry with retained form fields; concurrent normalized duplicates; persistence after a new Mongo client; durable rate-limit rollback with no raw IP; private CSV export, exact-contact deletion and logical expiry renewal; and mobile/auth isolation. Report: `tools/reports/public-launch-761bfad0eb/report.json`. Coming-soon desktop and saved mobile screenshots were visually inspected.

Only synthetic contacts were used. The readiness transport was controlled locally; it did not contact an external workspace. Browser external requests, private authentication headers/cookies, model calls and outreach were zero. The ownership-marked test database, synthetic CSVs, browser/HTTP server and owned Mongo process were cleaned; existing Sillage/Founder preview services and data were untouched. CI has a separate public-launch job using the same harness and a disposable labelled Mongo container; local workflow lint passes, but hosted CI execution is separate evidence.

**Not deployed:** the pinned Vercel **59.16.0** CLI reports logged out. No Vercel account/project or hosted interest database was linked/provisioned, and no production registration has been recorded. The operator must supply account/project access, a dedicated hosted database and bootstrap, a real monitored privacy contact and server settings, then verify actual hosted routing/storage. The [frontend operator guide](../apps/guardian/frontend/README.md#public-vercel-deployment) gives exact setup and export/deletion commands. Public early-access registration does not implement verified sign-in, self-service workspace provisioning, complete workflow/RAG capture or the broader production release gates.
