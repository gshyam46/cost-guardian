# Sillage: phase status

**Current R2-02 continuation: OpenTelemetry/OpenInference, ADR-51.** The 0.2.0 Python client adds an in-process numeric span processor, selected upstream instrumentation and an existing-provider attachment path under [OPENTELEMETRY.md](OPENTELEMETRY.md). Connections leads with the standards option while the CLI retains native compatibility by default. Real IDs, supplied agent context and honest missing-parent references extend the existing ledger; no full OTLP receiver or workflow-span datastore is introduced. The 67-test package suite and installed-wheel collector/worker acceptance pass locally, including OpenAI 1.x/2.x and native compatibility. This does not close R2, partner activation or production operating gates. Final product/build evidence belongs in [VALIDATION.md](VALIDATION.md).

The following ADR-50 and experience entries record earlier completed slices; their test results do not certify the current standards integration.

The current standards acceptance has an explicit coverage gate: four early-closed streams and one cancellation per OpenAI version produced no ended upstream span and therefore no captured call. Some returned/completed OpenAI Responses and LiteLLM returns are captured with unknown outcome instead. R2 cannot claim full stream/cancellation visibility until those upstream gaps or a verified compatible fallback are resolved. Overall regression/build totals are recorded in [VALIDATION.md](VALIDATION.md).

R2-02 now includes the installable Python/startup instrumentation slice defined before code in [INSTRUMENTATION.md](INSTRUMENTATION.md), ADR-50 and [PLAN.md](PLAN.md). This closes the copied-helper/per-call-edit requirement for tested Python interfaces. It does not close R2 as a whole: wider SDK/framework coverage, managed provisioning and independent customer activation remain gates. Current evidence is in [VALIDATION.md](VALIDATION.md).

2026-09-13 experience continuation: R2-03 now includes the public landing/demo and guided Connections with downloadable helpers; R1-03 adds an independent recent-call Overview and accurate failed/zero-duration display. The product name is Sillage under ADR-49. [EXPERIENCE.md](EXPERIENCE.md) defines the scope and [VALIDATION.md](VALIDATION.md) records current checks. This does not close R2's real customer activation or R5's self-service account/provisioning gates.
...
| R2 | Safe onboarding without observability; BYO alternative | In progress: isolated access, keys/native capture and selected OpenInference/OTel projection verified locally; general OTLP reception and managed provisioning remain planned | Both journeys with real data; supported Python/JS paths; role/privacy/lifecycle checks |
...
- R2: R2-01 isolated-project OIDC/session/role foundation implemented locally under [IDENTITY.md](IDENTITY.md); scoped credentials and native numeric capture are implemented locally under [CAPTURE.md](CAPTURE.md). Membership/session administration and deployed provider acceptance remain open; R2-02 includes Python/Node background exporters, explicit OpenAI helpers and the current selected OpenInference/OTel projection awaiting final acceptance. General OTLP reception, backing provisioning and broader provider/framework coverage remain planned. R2-03 access/setup recovery is implemented locally under [ACCESS.md](ACCESS.md), with BYO provisioning/privacy acceptance still open. Local verification does not close the real onboarding gate.
